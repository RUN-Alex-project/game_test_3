...
func _ready() -> void:
	...
	await _check("adventurer_roster", main.adventurer_roster_panel, "show")
	await _check("adventurer_mail", main.adventurer_mail_panel, "show")
	await _check("adventurer_trade", main.adventurer_trade_panel, "show")
	await _check("ranking", main.ranking_panel, "show")
	await _check("arena", main.arena_panel, "show")
	await _check("guild_market", main.guild_market_panel, "show")
	await _check("property_territory", main.property_territory_panel, "show")
	await _check("border_command", main.border_command_panel, "show")
	await _check("ice_codex", main.ice_codex_panel, "show")
	await _check("abyss_board", main.abyss_board_panel, "show")
	await _check("challenge_board", main.challenge_board_panel, "show")
	await _check("pet_endgame", main.pet_endgame_panel, "show")
	await _check("season_board", main.season_board_panel, "show")

	assert(exceed == 0, "one or more panels exceed viewport")
	print("PANEL_BOUNDS exceed=%d (all 24 panels within 700x550)" % exceed)
	...
	await _assert_close("adventurer_roster", main.adventurer_roster_panel)
	await _assert_close("adventurer_mail", main.adventurer_mail_panel)
	await _assert_close("adventurer_trade", main.adventurer_trade_panel)
	await _assert_close("ranking", main.ranking_panel)
	await _assert_close("arena", main.arena_panel)
	await _assert_close("guild_market", main.guild_market_panel)
	await _assert_close("property_territory", main.property_territory_panel)
	await _assert_close("border_command", main.border_command_panel)
	await _assert_close("ice_codex", main.ice_codex_panel)
	await _assert_close("abyss_board", main.abyss_board_panel)
	await _assert_close("challenge_board", main.challenge_board_panel)
	await _assert_close("pet_endgame", main.pet_endgame_panel)
	await _assert_close("season_board", main.season_board_panel)
